...
plot(x_seq, x_seq^2/2, type = "l", lwd = 3, xlab = "$(y - \\mu)/\\sigma^2$", ylab = "$\\ell(y; \\theta, \\kappa_2)$", col = "black", ylim = c(0, 4))
...
plot(x_seq, dnorm(x_seq, 0, 1), type = "l", lwd = 3, xlab = "$(y - \\mu)/\\sigma^2$", ylab = "$\\tilde{f}(y; \\theta, \\kappa_2)$", col = "black")
for(i in 1:length(kappa_thresh)){
...
# , verbose = TRUE
  ...
  cat("Parameter kappa = ", kappa_vect[k], "done, $\mathcal{H}$-score = ", hat_H_score_Tukeys_nu_marginal_absapprox_cont_n500[k],"\n")
}
...
Producing the plots for Figure 2 of the paper.
